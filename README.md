...
* `CLIManager`
    - *(фаза 1 и 2)* класс, который предоставляет методы:
        + `run` - осуществляет парсинг и выполнение команды получая ее как строчку
        + `execute` *(фаза 1)* - принимает на вход токены, полученные из `ContentParser`. Затем выполняет `Keyword`, который находится на первой позиции в списке, передавая ему список из оставшихся `CLIEntity`. В остальных случаях кидаем `RunException`.
        + `execute` *(фаза 2)* - ппринимает на вход токены, полученные из `ContentParser`. Затем итеративно разбирает `List<ICLIEntity>`. На каждой итерации мы повторяем такую же логику, как во время фазы 1, но `Keyword`'у передаются аргументы до `PipeArgument` и результат выполнения на предыдущей итерации. Если итерацию не получилось выполнить, то кидаем `RunException`.
